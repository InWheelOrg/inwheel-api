...
| Package | Role |
|---|---|
| `pkg/models` | Shared domain types (`Place`, `AccessibilityProfile`, `EntranceProps`, `PathwayProps`, `RestroomProps`, `ParkingProps`, `ElevatorProps`, `SourceReport`, `ExternalRef`, `UnmatchedExternal`, `APIKey`). JSONB customs (`Geometry`, `PlaceTags`, `ExternalIDs`, each Props type) implement `driver.Valuer`/`sql.Scanner`. |
| `internal/a11y` | Audit-flag computation per typed component, `ComputeEffectiveProfile` for parent inheritance (sets `IsInherited`/`SourceID` at read time, never stored). No conflict detection — flags are informational only. |
...
| `internal/sources` + `internal/sources/osm` | Capability interfaces and OSM concrete source (tag allowlist, transformer, PBF streamer). OSM wheelchair tags become `SourceReports` on the profile. |
